...
class WhatsAppClient(BaseOutboundProvider):
    ...
    async def send_message(self, payload: OutboundMessageRequest) -> OutboundMessageResult:
        if (
            ...
            form_payload = {
                "From": self.settings.twilio_whatsapp_number,
                "To": payload.to,
        ...
        logger.info("Outbound WhatsApp message stub", extra={"to": payload.to})
        return OutboundMessageResult(
            channel=payload.channel,
            provider=self.provider,
            to=payload.to,
